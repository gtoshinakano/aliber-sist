...
const Statistics = ({ title, data, config, deposits }) => {
  ...
  const balances = balancesQuery.isFetched ? balancesQuery.data.filter(i => i[0] === config.mes_ref && i[1] === parseInt(config.ano_ref)) : []
      ...
      {balances[0] && balances[0][2] > 0 && balances[0][2] > 0 && <div className='w-full pl-2 mt-2'>
        <div className='flex flex-col'>
          ...
          <div className='inline-flex justify-between'>
            ...
            <span>{balances[0] && balances[0][3] ? balances[0][3].toFixed(2).replace(".", ",") : 0.00}</span>
          </div>
          <div className='inline-flex justify-between'>
            <b>TOTAL</b>
            <span>{balances[0] && balances[0][3] && balances[0][2] ? (balances[0][2] + balances[0][3]).toFixed(2).replace(".", ",") : balances[0][3] || balances[0][2] || 0.00}</span>
          </div>
        </div>
      </div>}
    </div>
